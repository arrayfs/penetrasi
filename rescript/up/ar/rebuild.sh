...
git clone --depth=1 -j$(nproc --all) -b nethunter https://github.com/Correctl/msm8917-Riva-IDN --single-branch Xernethun && cd Xernethun
git clone https://github.com/LineageOS/android_prebuilts_gcc_linux-x86_aarch64_aarch64-linux-android-4.9 -b lineage-18.1 tc --depth=1 --single-branch
...
curl -F document=@$ZIPNAME.zip "https://api.telegram.org/bot$TOKEN/sendDocument" \
        ...
        -F "disable_web_page_preview=true" \
        -F "parse_mode=html"  \
	-F caption="#t.me/random_IDN #nethunter follow @Arrayfs for more updates"
